# 5_read_qualtrics_v2.R
# read the data directly from qualtrics
# version for second DCE survey
# May 2024
library(dplyr)
library(qualtRics)
library(janitor)
...
names = read_excel('data/5_qualtrics_labels_v2.xlsx', col_names = FALSE, n_max = 1) %>% 
  clean_names()
names = janitor::make_clean_names(names)  
# ... then get the labels
in_labels = read_excel('data/5_qualtrics_labels_v2.xlsx', col_names = FALSE, skip=1, n_max = 1)
...
surveys = all_surveys() %>%
  filter(str_detect(name, pattern='^Second DCE')) # just the DCE surveys
...
data = mutate(raw, 
              ...
              q31 = as.numeric(q31), # target papers
              q32 = str_squish(q32)) %>%  # comments
  select(-recorded_date, # just use start_date
         -starts_with('q24_do')) # do not need the randomised order for questions 24
...
### save ###
# main version
comment_questions = c('q25_8_text','q32')
data = select(data, -recipient_first_name, -recipient_last_name, -recipient_email) # not needed
save(data, labels, comment_questions, file='data/5_AnalysisReady_v2.RData')
...
#write.table(for_csv, file='data/tab_data_v2.txt', sep='\t', quote = FALSE, row.names = FALSE) # tab format
write.table(for_csv, file='data/tab_data_v2.csv', sep=',', quote = FALSE, row.names = FALSE) # csv format
# export labels
source('99_functions.R')
labels = mutate(labels, label2 = nice_rename(names))
write.table(labels, file='data/labels_v2.txt', sep='\t', quote = FALSE, row.names = FALSE)
