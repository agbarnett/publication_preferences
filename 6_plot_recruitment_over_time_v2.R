# 6_plot_recruitment_over_time_v2.R
# plot the recruitment by journal over time (for second survey)
# called from 6_summary_v2.Rmd
# May 2024
library(ggplot2)
library(dplyr)
...
# plot
label1 = data.frame(start_date = as.Date('2024-05-07'), csum=20, label='First 6374 sent')
label2 = data.frame(start_date = as.Date('2024-05-14'), csum=20, label='First 6049 reminder')
label3 = data.frame(start_date = as.Date('2024-05-21'), csum=20, label='Second 5841 reminder')
gplot = ggplot(data=to_plot, aes(x=start_date, y=csum)) +
  geom_step(linewidth=1.05, col='darkseagreen4')+
  geom_text(data = label1, aes(x=start_date, y=csum, label=label), adj=0, nudge_x=-0.5, angle=90, col='grey55')+
  geom_text(data = label2, aes(x=start_date, y=csum, label=label), adj=0, angle=90, col='grey55')+
  geom_text(data = label3, aes(x=start_date, y=csum, label=label), adj=0, angle=90, col='grey55')+
...
#
jpeg('figures/recruitment_v2.jpg', width=5, height=4, units='in', res=500, quality = 100)
print(gplot)
dev.off()
