---
title: "Summary of DCE responses (pilot version of DCE survey)"
...
```{r}
# total number sent (first two combined)
n_sent = filter(sent, sample!='First main') %>% pull(n_sent) %>% sum()
...
## Alternative plot of missing data ordered by missingness

```{r}
#
mplot_alternative = vis_miss(for_missing, cluster = TRUE) +
  theme(legend.position = 'bottom',
        plot.title = element_text(margin = margin(0,0,-10,0))) + # reduce space between title and plot
  scale_y_continuous(expand=c(0,0)) +
 # scale_x_discrete(breaks = xbreaks)+#, labels = xlabels)+ # not working
